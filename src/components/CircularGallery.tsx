// @ts-nocheck — third-party ReactBits component, skip strict TS checks
...
class App {
  ...
  update(time?: number) {
    // Auto-play: continuously increment target for slow rotation
    if (this.autoplaySpeed > 0) {
      const now = time ?? performance.now();
      const dt = this.lastTime ? Math.min((now - this.lastTime) / 16.67, 3) : 1;
      this.lastTime = now;
      this.scroll.target += this.autoplaySpeed * dt * 0.01;
    }
